...
export default async function ProjectPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <main id="main" className="project-page">
    <section className="project-hero"><div className="container"><Link href="/work" className="back-link">← All work</Link><div className="project-title"><div><p className="kicker">{project.eyebrow} · {project.year}</p><h1>{project.title}</h1></div>{hasDuration && <span>{project.duration}</span>}</div>{hasVideo ? <VideoPlayer title={project.title} poster={project.poster} sources={project.sources} /> : <div className="project-poster-wrap"><Image src={project.poster || project.thumbnail} alt={`${project.title} poster`} fill sizes="100vw" priority unoptimized={Boolean(project.poster?.startsWith("http") || project.thumbnail?.startsWith("http"))} /></div>}</div></section>
    <section className="project-details"><div className="container project-details-grid"><div><p className="kicker">The project</p><p className="project-lead">{project.longDescription}</p></div><dl><div><dt>Year</dt><dd>{project.year}</dd></div>{hasDuration && <div><dt>Duration</dt><dd>{project.duration}</dd></div>}<div><dt>Role</dt><dd>{project.role}</dd></div><div><dt>Tools</dt><dd>{(project.tools ?? []).join(" · ") || "Not specified"}</dd></div></dl></div></section>
  </main>;
}
